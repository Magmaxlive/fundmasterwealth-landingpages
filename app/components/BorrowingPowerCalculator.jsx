...
export default function BorrowingPowerCalculator({
  ...
  lede = 'Thinking about buying your first home? Use our borrowing power calculator to get an initial estimate based on your income, deposit and financial commitments.',
  sidebarHeading = 'Want to know what you could realistically buy?',
  sidebarLines = [
    'A borrowing calculator can give you a starting point. A personalised assessment can give you a clearer picture of what may actually be possible.',
    'Speak with aFundmaster Wealth adviser about your circumstances.',
  ],
  sidebarCtaLabel = 'Speak to aFundmaster Wealth Adviser',
  sidebarCtaHref = '#check',
  formHead = 'Calculator',
  submitLabel = 'Calculate My Borrowing Power',
  errorMsg = 'Please enter your income to see an estimate.',
  resultLabel = 'Estimated borrowing power',
  resultSubtext = 'Indicative only — not an offer of lending.',
  priceLabel = 'Indicative purchase price',
  repayLabel = 'Est. weekly repayment',
  disclaimer = 'Calculator results are indicative only and do not constitute financial advice or lending approval.',
}) {
    ...
    <section className="section section--dark" id={id}>
      <div className="wrap calc">
        <div className="rv">
          ...
          {sidebarHeading || (sidebarLines && sidebarLines.length) || sidebarCtaLabel ? (
            <div style={{ marginTop: '34px', padding: '26px 28px', borderRadius: '18px', background: 'rgba(255,255,255,.05)', border: '1px solid rgba(255,255,255,.12)' }}>
              ...
              {sidebarLines && sidebarLines.map((line, i) => (
                <p key={i} style={{ margin: i === sidebarLines.length - 1 ? '0 0 20px' : '0 0 8px', fontSize: '15px', color: 'rgba(232,240,248,.75)' }}>{line}</p>
              ))}
        ...
        <div className="calc__panel calc__panel--dark rv">
          ...
          <form noValidate onSubmit={handleSubmit}>
            ...
            <p className={`calc-err${error ? ' is-on' : ''}`} id="f-err">{errorMsg}</p>
          </form>
          ...
          <div ref={resultRef} className={`result${result ? ' is-open' : ''}`} id="f-res">
            ...
            <div className="result__split">
              <div className="result__cell"><b id="f-price">{result ? result.price : '$0'}</b><span>{priceLabel}</span></div>
              <div className="result__cell"><b id="f-repay">{result ? result.repay : '$0'}</b><span>{repayLabel}</span></div>
            </div>
          </div>
          {disclaimer ? (
